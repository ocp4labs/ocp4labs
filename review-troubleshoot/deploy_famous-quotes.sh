...
if ! oc exec ${MYPOD} -n ${PROJECT} -- /opt/rh/rh-mysql57/root/usr/bin/mysql -u root -e 'show databases;' | grep -q defaultdb
then
  echo "Creating famous-quotes database"
  oc exec ${MYPOD} -n ${PROJECT} -- /opt/rh/rh-mysql57/root/usr/bin/mysql -u root -e 'CREATE DATABASE defaultdb'
fi
